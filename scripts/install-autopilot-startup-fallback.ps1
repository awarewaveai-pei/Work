...
$startupCmd = @(
    "@echo off",
    "powershell -WindowStyle Hidden -NonInteractive -NoProfile -ExecutionPolicy Bypass -File ""$WorkRoot\scripts\ao-resume.ps1"" -WorkRoot ""$WorkRoot"" -SkipVerify -AllowUnexpectedDirty",
    "if errorlevel 1 (",
...
$loopCmd = @(
    "@echo off",
    "powershell -WindowStyle Hidden -NonInteractive -NoProfile -ExecutionPolicy Bypass -File ""$WorkRoot\scripts\autopilot-alert-loop.ps1"" -WorkRoot ""$WorkRoot"" -IntervalSeconds 600"
) -join "`r`n"
