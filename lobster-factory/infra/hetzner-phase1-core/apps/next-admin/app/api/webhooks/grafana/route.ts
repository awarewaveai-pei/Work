...
import { normalizeGrafana } from "@/lib/ops-inbox/normalize/grafana";
...
export async function POST(req: Request) {
  ...
  const draft = normalizeGrafana(raw);
  ...
  if (error || !upserted) {
    Sentry.captureException(error ?? new Error("upsert_empty"), { tags: { route: "ops-inbox-webhook-grafana" } });
