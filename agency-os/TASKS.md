...
- [ ] **（AO-RESUME 提醒）雙機環境對齊（桌機＋筆電）**：**（2026-04-06）**筆電已 **`git push origin main`**（細節與「不得僅靠 Git 同步」清單見 **`WORKLOG.md` 當日「雙機／遠端」**）；**公司桌機上班第一件事**：monorepo 根 **`git pull --ff-only origin main`** 至與 **`origin/main` 一致**，再依 [`docs/overview/REMOTE_WORKSTATION_STARTUP.md`](docs/overview/REMOTE_WORKSTATION_STARTUP.md) **§2**（`packages\workflows` 之 **`npm ci`**、`verify-build-gates`、再 **`AO-RESUME`**）。兩台「版本庫」靠 Git；**`.env.local`／vault／`mcp.json`／本機 WP DB** 仍各機維護（見 **`hetzner-self-host-start-here.md`** 環境變數對照與 **§1.5.1**）。**新機或重裝**仍從 **§1.5** 起。要點：**`gh`**、**Node** 與 CI、**每台** vault／MCP。桌機跑完 §2 且與筆電能力一致後勾選本項。
