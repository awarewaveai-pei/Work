...
- [ ] **（AO-RESUME 提醒）雙機環境對齊（桌機＋筆電）**（詳見 `docs/overview/REMOTE_WORKSTATION_STARTUP.md` §1.5 / §1.5.1）
  ...
  - **Trigger.dev（自託管）**：堆疊政策與缺口見 **`docs/operations/hetzner-stack-rollout-index.md`**（**Trigger.dev（僅自託管）**）、步驟級 **`docs/operations/hetzner-full-stack-self-host-runbook.md`**（階段 3）；現況能力列 **`docs/operations/TOOLS_DELIVERY_TRACEABILITY.md`**（**🟡 建置中**）。**每台開發機**：`lobster-factory/packages/workflows` 之 **`TRIGGER_SECRET_KEY`、API／Worker base URL**（或等同 env）須在該機 **vault**／本機 env 對齊自架叢集；**GitHub Actions × Trigger deploy** 與自架切線見 **`docs/operations/github-actions-trigger-prod-deploy.md`**（避免以為「CI 綠燈」等於「自架 Worker 已接上」）。
