...
- 使用者輸入 `AO-RESUME` 時：**Cursor 專案 hook 會先真的跑腳本**——monorepo 根 **`.cursor/hooks.json`** 的 **`beforeSubmitPrompt`** 會呼叫 **`ao-resume.ps1 -FullMainlineParity`** 並寫入 **`agency-os/.agency-state/ao-resume-hook-last.json`**／**`.log`**（見 **`30-resume-keyword.mdc`** 去重條款）。之後 **預設＝代理包办**：代理依該規則決定是否仍需終端補跑，並讀 **`open-tasks-snapshot.md`**、`TASKS`、`WORKLOG`、`CONVERSATION_MEMORY` 等回五段式；**禁止**在代理可自跑成功時仍叫使用者「你先手動 pull／先貼指令」。**無終端**時依該規則極窄義 fallback，並標註未跑完整腳本。
...
- 若已啟用 Autopilot Phase1，開機會自動執行 `scripts/ao-resume.ps1 -SkipVerify -SkipStrictEnvironmentAudit -AllowUnexpectedDirty`（輕量 preflight；**不**跑完整閘道與 Strict 環境稽核；**不**取代你主動打 **`AO-RESUME`** 時應跑的完整 **`ao-resume.ps1 -FullMainlineParity`**）。
