...
- 使用者輸入 `AO-RESUME` 時：**預設＝代理包办**——**可執行終端下**須由代理先跑 **`scripts/ao-resume.ps1 -FullMainlineParity` 至 exit 0**（見 **`30-resume-keyword.mdc`**），再讀 **`open-tasks-snapshot.md`**、`TASKS`、`WORKLOG`、`CONVERSATION_MEMORY` 等並回五段式；**禁止**在代理可自跑成功時仍叫使用者「你先手動 pull／先貼指令」。**無終端**時依該規則極窄義 fallback，並標註未跑完整腳本。
