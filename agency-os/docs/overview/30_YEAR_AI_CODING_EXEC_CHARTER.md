...
## 2. 工程層執行（Execution）
1. **每日續接**：預設在 Cursor 開 monorepo 根並送 **`AO-RESUME`**；**hook** 先跑 **`scripts/ao-resume.ps1 -FullMainlineParity`**（behind 時才 ff-only pull＋閘道＋Strict 稽核）再讀檔；髒樹／衝突時腳本會非 0，請先整理（見 **`REMOTE_WORKSTATION_STARTUP` 2.5.1**）。除錯時可改手動跑腳本。
