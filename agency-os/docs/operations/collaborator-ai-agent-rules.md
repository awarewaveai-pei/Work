...
- **變更路徑（可多行）**:
  - `path/to/file`
- **Git（若已 commit）**: `<short-or-full-hash>` 或 branch 名 +「未 push」（多個 hash 可用子清單列出）
...
**僅六個頂層欄位**：禁止新增範本未列之 `- **…**:` 頂層列（例如「伺服器操作（無 commit）」）。無 git 的遠端操作請寫進 **風險／待辦**（括註遠端／無 commit）或併入 **變更路徑**／**完成（一句）**。

**對應 TASKS**：子字串須在**恰好一條**仍為 `- [ ]` 的 TASK 行內可找到；若會匹配多條，改寫更長的唯一片段。**禁止**以逗號並列多個無關項目（如 `a, b, c`）；若本輪對應多條待辦，請拆成多個 `###` 區塊或只填主線一條。
